...
and with (crucially)~$\vec{v}=\vec{v_0}$ and~$\vec{t}=-\rho(B)\vec{v_0}$, and applying a little algebra, we obtain
the desired result.
\qed
\end{example}
...
Let $A$ be a quantifier-free type in which every
occurrence of $\tyPrimNm{vec}$ is of the form $\tyPrim{vec}{B,e}$ for some $e$. Then
\[
...
and indices on primitives; also note that the omitted cases for $\transup A t v$ 
...
&
\transup{t} t v &= \lambda w.w + (\mathrm{negate}\, v)
\\
...
induction on $A$ using the fact that the vector $(0,0)$
is an identity for vector addition. To show $J(I(x))$ is contextually
equivalent to $x$ we first prove (by induction on $A$) 
that for any $\rho\in\semSort{\DeltaWithB}$, and any $\vec v\in\mathbb R^2$, 
the graph of the function given by $\tmSem{\transdn A t v}(\opsEnv,\vec v)$
is just  $\rsem A (\rho,\vec v)$, and conversely, the graph of $\tmSem{\transup A t v}(\opsEnv,\vec v)$
is ${\rsem A (\rho,\vec v)}^{\mathrm{op}}$.
\end{proof}
