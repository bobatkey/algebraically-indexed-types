...
For index-erasure interpretation, we assume a (not necessarily abelian) group $(G, 1, \cdot,
...
We write $i^n$ for $n$-fold iteration of the group operation.
\begin{theorem}
...
from \exref{ex:uninhabited-type} is an instance of this theorem.

For types indexed by the scaling group $\SynGL{1}$ the situation is a little more complex, as
the index-erasure interpretation of $\tyPrim{real}{s}$ is larger than the group $\GL{1}$, as it contains
zero. We can show, nonetheless, that $\forall s \mathord:
  \SynGL{1}.\ \tyPrim{real}{s \circ s} \to \tyPrim{real}{s}\cong 1$, reflecting 
the fact that any term of this type must be the function $\lambda x.0$.
