...
We define the model of the indexing theory for the two-dimensional
geometry instantiation as follows. Each of the sorts is interpreted just as
its semantic counterpart: $\semSort{\SynTransl{2}} = \Transl{2}$, $
\semSort{\SynGL{2}} = \GL{2}$, $ \semSort{\SynOrth{2}} = \Orth{2}$,
and $ \semSort{\SynGL{1}} = \GL{1}$.
%  \begin{displaymath}
%    \begin{array}{@{}llll}
...
relational interpretations of $\tyPrim{vec}{B,t}$ and
$\tyPrim{real}{s}$, we use the compositional construction from
\autoref{sec:constr-rel-env}, with $R_{\tyPrimNm{vec}}(B,\vec{t}) = \{
(\vec{v}, B\vec{v} + \vec{t}) \sepbar \vec{v} \in \mathbb{R}^2 \}$ and
$R_{\tyPrimNm{real}}(k) = \{ (x, kx) \sepbar x \in \mathbb{R} \}$. The
...
\begin{lemma}\label{lem:geom-environments-1}
  For all $\Delta$ and $\rho \in \relEnv{E}_\Geom(\Delta)$, $(\eta_\Geom,\eta_\Geom) \in \rsem{\Gamma_\Geom}{\relEnv{E}_\Geom}\rho$.
\end{lemma}
...
primitive operations typed by $\Gamma_\Grp$ that reflect the group
structure at the value level: identity $1:\tyPrim{G}{1}$, operation
$(\cdot): \forall a,b\mathord:\mathsf{G}.\ \tyPrim{G}{a} \to
\tyPrim{G}{b} \to \tyPrim{G}{a\cdot b}$ and inverse $\ ^{-1}: \forall
a\mathord:\mathsf{G}.\ \tyPrim{G}{a} \to \tyPrim{G}{a^{-1}}$. For the
index-erasure semantics, we assume some (not-necessarily abelian)
group $G$ and $\tyPrimSem{\tyPrimNm{G}} = G$.  The interpretation
$\eta_\Grp$ of the primitive operations in $\Gamma_\Grp$ is just the
unit, multiplication and inverse operations of $G$. One instance is the
sort~$\SynTransl{2}$ with $0$, $(+)$ and $\mathrm{negate}$ on
$\tyPrim{vec}{t}$.

We use the construction of non-compositional relational environments
...
\begin{theorem}
  ...
  in the integers to the equation $A \vec x = b$.
\end{theorem}
