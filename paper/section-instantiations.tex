...
I & \isDefinedAs\lambda f\mathord:X.f_0\,0_0\\
J & \isDefinedAs\lambda v\mathord:Y.\Lambda t\mathord:\SynTransl{2}.\lambda w\mathord:\tyPrim{vec}{e,t}.w +_{e,t,0} v
\end{align*}
Unfolding definitions gives that $\tmSem{I(J(v))}(v,\opsEnv)$ = $0+v$
...
$(\tmSem{I(J(f))}(f,\opsEnv), f')\in\rsem X\rho$.
...
We can generalise this isomorphism to any quantifier-free (but possibly open) type
...
\exref{ex:type-iso} and \exref{ex:simpletrans} are special cases of this isomorphism; another instance is the
type of vector addition:
\[
\begin{array}{l}
\forall B\mathord:\SynGL{2}, t_1\mathord:\SynTransl{2}, t_2\mathord:\SynTransl{2}.\tyPrim{vec}{B,t_1}\to\tyPrim{vec}{B,t_2}\to\tyPrim{vec}{B,t_1+t_2}\\
\cong\quad
...
$I = \lambda f. f_0\ 0_0$ and $J = \lambda y. \Lambda t.\lambda v. \transup A t v (y)$,
%\begin{eqnarray*}
%I = \lambda x. x\ [0]\ 0 & \text{ and } &
%J = \lambda y. \Lambda t.\ \lambda v. \trans A v t(y)
%\end{eqnarray*}
where terms $\transdn A t v : A\to A[0/t]$ and $\transup A t v : A[0/t] \to A$ are defined by induction over the syntax of $A$ (cases omitted for $\transup A t v$ have definition symmetric to $\transdn A t v$):
\begin{align*}
\transdn{\tyPrimNm{unit}} t v &= \lambda x. x \qquad
...
\transdn{\tyPrim{vec}{e}} t v &= \lambda x. x - kv \qquad
\transup{\tyPrim{vec}{e}} t v = \lambda x. x + kv\\ 
