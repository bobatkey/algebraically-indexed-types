...
type. {\bf Vague. Say which invariance property.}
...
\begin{example}\label{ex:uninhabited-type}
  ...
  inhabitants. Intuitively, this is because there is no way to remove
  the extra $t$ in $\tyPrim{vec}{t+t}$ using the vector operations. We
  formalise this indefinability result in
  \autoref{sec:types-indexed-abelian-groups} using a more
  sophisticated relational interpretation of free translation
  variables than the one we sketched above.
\end{example}
...
analysis \fixme{ref} to bear on programming. The types of the real
number arithmetic operations in \autoref{fig:real-ops} are exactly the
types Kennedy assigns in his units of measure system, except for that
of the absolute value operation. Semantically, our type indexes by
...
\begin{example}
  ...
  proof and show that even if we add square root as a primitive
  ...
  related to the impossibility of trisecting an arbitrary angle by
