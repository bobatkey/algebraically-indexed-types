...
$\tyPrim{T}{e}$ of representations of the truth of a logical
proposition $e$. The relationships between security levels are now
replaced by logical entailment, so we only have functions of type
$\tyPrim{T}{\phi} \to \tyPrim{T}{\psi}$ when $\phi$ entails
$\psi$. %Inspired by the PER interpretation of information flow, 
We instantiate our general relational framework to interpret
$\tyPrim{T}{\phi}$ as the identity relation if $\phi$ is true and the
empty relation if $\phi$ is false. This recovers the information flow
properties of Abadi {\em et al.} and Tse and Zdancewic.
...
We adapt Tse and Zdancewic's translation of Abadi et al.'s monadic
...
data. Thus a program cannot get access to the exact value. From this
and \thmref{thm:abstraction}, we obtain the following information flow
result:
