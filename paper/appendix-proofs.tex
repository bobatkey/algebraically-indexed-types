...
\begin{proof}
  ...
  \item By induction on the derivation of $\Delta \vdash A \isType$. We
    ...
    \item[Case \TirName{TyForall}] We have $A = \forall
      ...
      $\rho_1 \circ \pi_{i:s} = \rho \circ \sigma$ (by the assumption
      that $\rho_1 \in \extends{\rho \circ \sigma}{i\mathord:s}$), so
      we can obtain a $\rho' \in \relEnv{E}(\Delta',i\mathord:s)$ such
      that $\rho' \circ \pi_{i\mathord:s} = \rho$ (so $\rho' \in
      ...
      required.
    \item[\TirName{TyExist}] We have $A = \exists i\mathord:s.A'$. We
      prove the required equation by demonstrating two
      inclusions. From left-to-right, we know that
      \begin{equation}
