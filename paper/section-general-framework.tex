...
\begin{example}[Two-Dimensional Geometry]
  Example~\autoref{ex:two-dim-geo-axioms}
...
suffices to prove free theorems and type isomorphisms for
instantiations of our general framework. However, to prove
non-definability results, like the one in
...
the algebraic indexing. In this section, we identify the 
properties we require of relational environments in order for
the abstraction theorem to hold. In \autoref{sec:constr-rel-env}, we
give a general construction of relational environments and use it 
to prove several non-definability results.
...
of relational environments that enforce different invariants. We
...
We use the relational interpretation to define the notion of semantic
equivalence, which is a sound approximation of contextual equivalence,
as a consequence of \thmref{thm:abstraction}.
\begin{definition}
...
relational environments 
%(in the sense of \defref{defn:valid-rel-env-family}) 
that allow us to derive
useful consequences of \thmref{thm:abstraction}, e.g.,
%in particular, 
%we wish to show 
that certain types are uninhabited. To %motivate 
this end, consider the
%following 
type $\forall t\mathord:\SynTransl{2}.\ \tyPrim{vec}{t + t} \to
\tyPrim{vec}{t}$ involving vectors indexed by translations.
% as in \autoref{sec:affine-vector-ops}.
%\begin{displaymath}
...
result type. More generally, our operations on vectors 
only allow us to combine indices according to the group operations. Thus,
if the index of the result type of a function does not lie within the
free abelian group generated by the indices of the input types, then
there is no way to write a program with this %given 
type. We now
develop this idea to generate a family of sets of relational
environments that is valid in the sense of
\defref{defn:valid-rel-env-family}. The key idea is to assign
different relational interpretations to types with indices that lie
outside some fintely generated group of index expressions.
...
defined as follows. To each sort $s \in \SortSet$ we assign a carrier
...
\semIndexExp{\cdot}_{\mathcal{M}})$ 
%such that all the axioms in
satisfying all the axioms in
$\IndexAxiomSet$.
% are satisfied.
...
$\mathcal{M}$ to provide provide the underlying interpretation of
index expressions. %As we indicated above, we wish 
To discriminate
between index expressions that are generated from some finite set of
``input'' expressions and those that are not, %. Accordingly, 
we assume that for each primitive type $\tyPrimNm{X}$ we have two ways of
assigning a relational meaning to that type. When the index arguments
of the type are within the generated set, we use a function
$R_{\tyPrimNm{X}} \in
...
%construct the relational environments that we are interested in.  
Now, fix
an index context $\Delta$, and let $A = \{ A_s \}_{s \in \SortSet}$ be a
...
\fixme{This is the content of lemma X, lemma Y, lemma Z, below}.
