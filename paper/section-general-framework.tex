...
We give two constructions of families $\relEnv{E}$ of sets of
relational environments that allow us to derive useful consequences of
\thmref{thm:abstraction}. To satisfy the hypothesis of
...
Our model-based relational environments are defined in terms of
...
\paragraph{Definability Relational Environments}
We now construct relational environments that can be used for
non-definability results. To this end, consider the typing
$t\mathord:\SynTransl{2}; \Gamma \vdash M : \tyPrim{vec}{t + t} \to
\tyPrim{vec}{t}$, where $\Gamma$ contains the primitive operations on
vectors indexed by translations from \autoref{sec:affine-vector-ops}.
In \exref{ex:uninhabited-type} we claimed that this type is
uninhabited. Intuitively, it is clear why: with the operations on
vectors in \autoref{sec:affine-vector-ops}, there is no way of
cancelling the additional $t$ in the argument type to get the result
type. More generally, the vector operations only allow us to combine
indices according to the group operations, so if the index of the
result type of a function lies outside the free abelian group
generated by the indices of the input types, then we cannot write a
program of this type.  The key idea is to assign different relational
interpretations to types with indices not in some generated
collection of index expressions.
...
Relational environments are parameterised by sets of terms $S$:
\begin{displaymath}
  ...
  \right.
\end{displaymath}
Thus we assign the equality relation to primitive types whose index
arguments are generated up to equivalence by $S$. For types whose
indices are not in $\Gen^\equiv(S)$, we require a binary relation
$R^\bullet_{\tyPrimNm{X}} \in \Rel(\tyPrimSem{\tyPrimNm{X}})$. We set
$\relEnv{E}_{R^\bullet}(\Delta) = \{ \rho^S \sepbar \forall e \in S.\
\exists s. \Delta \vdash e : s \}$.

We have not been able to prove that this family of sets of relational
interpretations is substitutive. Therefore our non-definability
results are subject to the quantifier-free conditions in
\thmref{thm:abstraction}.

% Hence we must make do with the
% following theorem:
