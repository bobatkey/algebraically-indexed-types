...
sort. Existential types are constructed using the \TirName{TyExist}
...
\begin{example}[Two-Dimensional Geometry]
  \fixme{Tie this example back to section 2}
...
of \emph{relational environments}, which we now define.

A relation environment for a context $\Delta$ is a (dependently typed)
...
\item If we have a pair of relation environments $\rho_1 \in
  ...
  \relEnv{E}(\Delta)$, along with a simultaneous substitution $\Delta'
  \vdash \sigma \Rightarrow \Delta$, such that $\rho_1 \circ
...
\begin{figure*}[t]
  ...
  \begin{mathpar}
    ...
    {\Delta; \Gamma \vdash \mathrm{pack}\ [e]\ M : \exists i\mathord:s. A}
    ...
    {\Delta; \Gamma \vdash \mathrm{unpack}\ [i]\ x = M\ \mathrm{in}\ N : B}
  \end{mathpar}
...
must extend the relational interpretation of types to typing
