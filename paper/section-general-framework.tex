...
\exref{ex:uninhabited-type}), we choose a more subtle interpretation
of index contexts and primitive types that can account more precisely
for the definable programs permitted by the algebraic indexing. In
this section, we set out the general requirements that we require of
relational environments in order for the abstraction theorem to
hold. In \autoref{sec:constr-rel-env}, we give a general construction
of relational environments that will allow us to prove several
...
\subsubsection{A General Construction of Relational Environments}
...
relational environments that will allow us to derive useful
consequences of the abstraction theorem, including free theorem, type
isomorphisms and non-definability results. We make use of some
concepts of universal algebra, which we recall here. \fixme{Desperately needs some intuition: do so in terms of clumpiness}

\paragraph{Some Universal Algebra}
For each sort $s \in \SortSet$ we assume a carrier set
...
% A sub-model $\mathcal{N}$ of a given
% model $\mathcal{M}$ (written $\mathcal{N} \subseteq \mathcal{M}$) is a
% model such that $\semSort{s}_{\mathcal{N}} \subseteq
% \semSort{s}_{\mathcal{M}}$ for all $s \in \SortSet$.

\fixme{Define free models for any sort-indexed collection of sets}
Given an index context $\Delta$, the \emph{free} model on $\Delta$,
$\Free(\Delta)$, has $\semSort{s}_{\Free(\Delta)} = \{e \sepbar \Delta
\vdash e : s\}/\mathord\equiv$: the set of expressions of sort $s$ in
the indexing context $\Delta$, quotiented by the equality
judgment. The free model has
$\semIndexExp{\texttt{f}}_{\Free(\Delta)}(e_1,..,e_n) =
\texttt{f}(e_1,...,e_n)$.
...
\paragraph{Relational Environments} \fixme{Say some stuff in english to make sure that the construction is understandable}

To assign a relational meaning to
each of the primitive types we assume a some model $\mathcal{M}$ of
the indexing theory along with a function $R_{\tyPrimNm{X}} :
\semSort{\primTyArity(\tyPrimNm{X})}_{\mathcal{M}} \to
\Rel(\tyPrimSem{\tyPrimNm{X}})$ for each primitive type
$\tyPrimNm{X}$, and a constant $R^\bullet_{\tyPrimNm{X}} \in
\Rel(\tyPrimSem{\tyPrimNm{X}})$. We set
$\relEnv{E}_{\mathcal{M},R}(\Delta) = \{ \rho^{\mathcal{A},h} \sepbar \mathcal{A}
\subseteq \Free(\Delta), h : \mathcal{A} \to \mathcal{M} \}$, where:
\begin{displaymath}
  \rho^{\mathcal{A},h}\ \tyPrimNm{X}\ (e_1,...,e_n) = \left\{
    \begin{array}{ll}
      R^\bullet_{\tyPrimNm{X}} & \textrm{if any }e_i \not\in \mathcal{A} \\
      R_{\tyPrimNm{X}}(h(e_1),...,h(e_n)) & \textrm{if all }e_i \in \mathcal{A}
    \end{array}
  \right.
\end{displaymath}

\begin{lemma}
  The family $\relEnv{E}_{\mathcal{M},R}$ of sets of relation
  environments is valid (c.f.~\defref{defn:valid-rel-env-family}).
\end{lemma}

% \fixme{Note that we can recover the basic approach by restricting the
%   sub-algebras to just the whole thing. Does this work in general?
%   What about arbitrary collections of sub-algebras?}
...
\begin{example}[Two-dimensional Geometry] We define the ``standard''
  ...
  are as may be expected given the informal definitions in
