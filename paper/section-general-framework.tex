...
\begin{example*}[Geometry: interpretation]
  ...
  in the standard way. \todo{Expand}. 
\qed
\end{example*}
...
clearly satisfy the axioms in \exref{ex:two-dim-geo-axioms}. 
\qed
\end{example*}
...
%  \rsem{\Delta \vdash A \isType}M R & : & \semSort\Delta  \to \Rel(\tyPrimSem{A})\\
  ...
  \bigcap\{ \rsem{A} {(m,\rho)} \sepbar m \in \semSort{s} \} \\
  \rsem{\exists i\mathord:s.A} \rho & \isDefinedAs & 
  \bigcup\{ \rsem{A} {(m,\rho)} \sepbar m \in \semSort{s} \}
\end{eqnarray*}
...
Lemma~\autoref{lem:tyeqsubst-relational} (part 1) for rule \TirName{TyEq},
Lemma~\autoref{lem:tyeqsubst-relational} (part 2) for rules \TirName{UnivApp} and \TirName{ExPack},
and Lemma~\autoref{lem:ctxtsubst-rel} for rules \TirName{UnivAbs} and \TirName{ExUnpack}. 
...
equivalence is a sound approximation of contextual equivalence.
  ...
  \defref{defn:ctxt-equiv}. Fix a model and relational interpretation of primitive types so that
